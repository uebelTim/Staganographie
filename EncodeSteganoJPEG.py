...
def encodeJPEG():
    ...
    srcFileName = './Bilder/cover.webp'
    outputJPEGFileName = './Bilder/encodedStegano.jpg'
    quality = float(90)
    DEBUG_MODE = int(0)
    ...
    for y in range(0, imageHeight, 8):
        for x in range(0, imageWidth, 8):
            ...
            sosbitarray += bitarray(huffmanEncode.encodeDCToBoolList(dyDC[blockNum],1, DEBUG_MODE))
            # encode yAC
            if (DEBUG_MODE == 1):
                print("encode yAC:", yZCode[1:])
            yac = bitarray()
            huffmanEncode.encodeACBlock(sosbitarray, yZCode[1:], 1, DEBUG_MODE)
            
            # encode uDC
            if(DEBUG_MODE==1):
                print("encode duDC:",duDC[blockNum])
            #sosBitStream.write(huffmanEncode.encodeDCToBoolList(duDC[blockNum],0, DEBUG_MODE),bool)
            sosbitarray  += bitarray(huffmanEncode.encodeDCToBoolList(duDC[blockNum],0, DEBUG_MODE))
            # encode uAC
            if (DEBUG_MODE == 1):
                print("encode uAC:", uZCode[1:])
            uac = bitarray()
            huffmanEncode.encodeACBlock(sosbitarray, uZCode[1:], 0, DEBUG_MODE)
            

            # encode vDC
            if(DEBUG_MODE==1):
                print("encode vDC:",dvDC[blockNum])
            #sosBitStream.write(huffmanEncode.encodeDCToBoolList(dvDC[blockNum],0, DEBUG_MODE),bool)
            sosbitarray += bitarray(huffmanEncode.encodeDCToBoolList(dvDC[blockNum],0, DEBUG_MODE))
            # encode vAC
            if (DEBUG_MODE == 1):
                print("encode vAC:", vZCode[1:])
            vac = bitarray()
            huffmanEncode.encodeACBlock(sosbitarray, vZCode[1:], 0, DEBUG_MODE)
            

            blockNum = blockNum + 1

    return sosbitarray,srcImageWidth, srcImageHeight,luminanceQuantTbl,chrominanceQuantTbl



            

def writeJpegFile(outputFileName, srcImageWidth, srcImageHeight,sosbitarray,luminanceQuantTbl,chrominanceQuantTbl):
    jpegFile = open(outputFileName, 'wb+')
    # write jpeg header
    ...
    sosLength = len(sosbitarray) 
    filledNum = 8 - sosLength % 8
    if(filledNum!=0):
        for i in range(filledNum):
            sosbitarray.append(1)
        #print(numpy.ones(filledNum,dtype=int))
        #sosBitStream.write(numpy.ones([filledNum]).tolist(),bool)
       #sosbitarray += bitarray([numpy.ones(filledNum).tolist()])

    jpegFile.write(bytes([255, 218, 0, 12, 3, 1, 0, 2, 17, 3, 17, 0, 63, 0])) # FF DA 00 0C 03 01 00 02 11 03 11 00 3F 00

    # write encoded data
    sosBytes = sosbitarray.tobytes()
    for i in range(len(sosbitarray)//8):
...
def hideMessageInBitStream(encodedBitArray, message, Algorithm='JSteg'):
    print('len encodedBitArray:', len(encodedBitArray))
    bytes = encodedBitArray.tobytes()
    #print("bytes",bytes[0:1])
    n_bytes = len(bytes)
    #print('The maximum number of bytes to encode in this image is: {} = {} kB = {} mB'.format(n_bytes, int(n_bytes/1024),np.round( n_bytes/(1024*1024),3)))
    ...
    bin_message.frombytes(message.encode('ascii'))
    message_len = len(bin_message)
    #print number of bytes in message
    #print('The number of bytes in the message is: {}'.format(message_len // 8))
    message_index = 0

    encodedBytes = bitarray()
    for i in range(n_bytes):
        
        #print("i",i)
        #print('bytes[i] = ',bytes[i:i+1])
        byte = bitarray()
        byte.frombytes(bytes[i:i+1])
        if message_index < message_len:
            byte[7]=bin_message[message_index] #byte[7] is least significant bit
        encodedBytes += byte
        message_index += 1
        
    print('len encodedBytes:', len(encodedBytes))
    print('number of bytes in encodedBytes:', len(encodedBytes)//8)
    print("encodedBytes",encodedBytes)
    return encodedBytes



def decodeMessageFromJPEG(imageFile,algorithm='JSteg'):
    ...
    #find start creteria: 'FF DA 00 0C 03 01 00 02 11 03 11 00 3F 00' in b
    startindex = b.find(b'\xff\xda\x00\x0c\x03\x01\x00\x02\x11\x03\x11\x00\x3f\x00')
    startindex += 14
    print('startindex:', startindex)
    bin_message = ''
    ascii_message = ''
    for i in range(startindex, len(b)):
        byte = bitarray()
        byte.frombytes(b[i:i+1])
        bin_message += str(byte[7])
        if len(bin_message) % 8 == 0:
            ascii_message += chr(int(bin_message, 2))
            bin_message = ''
        if ascii_message[-5:] == '#####':
            break
    print('message decoded')

    return ascii_message[:-5]
...
if __name__ == '__main__':
    # encodedbitarray,w,h,YTable,CTable = encodeJPEG()
    # steganobitarray = hideMessageInBitStream(encodedbitarray, "Hello World")
    # writeJpegFile('./Bilder/encodedStegano.jpg',w,h, steganobitarray,YTable,CTable)
    message = decodeMessageFromJPEG('./Bilder/encodedStegano.jpg')
    print(message)